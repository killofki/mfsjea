...
const ALPHABET_QWERTY = new class { 
	name = 'Qwerty' 
	layout = LAYOUT_ALPHABET_QWERTY 
	} // -- ALPHABET_QWERTY 
const ALPHABET_DVORAK = new class { 
	name = 'Dvorak' 
	layout = LAYOUT_ALPHABET_DVORAK 
	} // -- ALPHABET_DVORAK 
const ALPHABET_COLEMAK = new class { 
	name = 'Colemak' 
	layout = LAYOUT_ALPHABET_COLEMAK 
	} // -- ALPHABET_COLEMAK 
...
const COMB_DUBEOL_STANDARD = new class { 
	cho = slashItems `` 
	jung = slashItems ` ㅗㅏ ㅘ / ㅗㅐ ㅙ / ㅗㅣ ㅚ / ㅜㅓ ㅝ / ㅜㅔ ㅞ / ㅜㅣ ㅟ / ㅡㅣ ㅢ ` 
	jong = slashItems ` ㄱㅅ ㄳ / ㄴㅈ ㄵ / ㄴㅎ ㄶ / ㄹㄱ ㄺ / ㄹㅁ ㄻ / ㄹㅂ ㄼ / ㄹㅅ ㄽ / ㄹㅌ ㄾ / ㄹㅍ ㄿ / ㄹㅎ ㅀ / ㅂㅅ ㅄ ` 
	} // -- COMB_DUBEOL_STANDARD 

const COMB_SEBEOL_390 = new class { 
	cho = slashItems ` ᄀᄀ ᄁ / ᄃᄃ ᄄ / ᄇᄇ ᄈ / ᄉᄉ ᄊ / ᄌᄌ ᄍ ` 
	jung = slashItems ` ᅩᅡ ᅪ / ᅩᅢ ᅫ / ᅩᅵ ᅬ / ᅮᅥ ᅯ / ᅮᅦ ᅰ / ᅮᅵ ᅱ / ᅳᅵ ᅴ ` 
	jong = slashItems ` ᆨᆨ ᆩ / ᆨᆺ ᆪ / ᆫᆽ ᆬ / ᆫᇂ ᆭ / ᆯᆨ ᆰ / ᆯᆷ ᆱ / ᆯᆸ ᆲ / ᆯᆺ ᆳ / ᆯᇀ ᆴ / ᆯᇁ ᆵ / ᆯᇂ ᆶ / ᆸᆺ ᆹ / ᆺᆺ ᆻ ` 
	} // -- COMB_SEBEOL_390 
const COMB_SEBEOL_FINAL = new class { 
	cho = slashItems ` ᄀᄀ ᄁ /  ᄃᄃ ᄄ / ᄇᄇ ᄈ / ᄉᄉ ᄊ / ᄌᄌ ᄍ ` 
	jung = slashItems ` ᅩᅡ ᅪ / ᅩᅢ ᅫ / ᅩᅵ ᅬ / ᅮᅥ ᅯ / ᅮᅦ ᅰ / ᅮᅵ ᅱ / ᅳᅵ ᅴ ` 
	jong = slashItems `` 
	} // -- COMB_SEBEOL_FINAL 
...
const jeamfsList = ( str, moachigi = true ) => 
	ALPHABET_LAYOUTS .flatMap( alphabet => 
		HANGUL_LAYOUTS .map( hangul => output( 
			  jeamfsORdudgks( str, alphabet, hangul, moachigi ) 
			, alphabet 
			, hangul 
			) ) // -- output // -- map 
		) // -- flatMap 
...
const jeamfsAuto = ( str, moachigi ) => { 
	let sorted = jeamfsList( str, moachigi ) 
...
module .exports = { jeamfsList, jeamfs: jeamfsAuto, count2350 } 
